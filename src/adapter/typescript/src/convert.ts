...
export function convertTypedoc(
  json: unknown,
...
function convertReflection(
  ...
  switch (refl.kind) {
    ...
    default:
      console.warn(
        `[adapter-typescript] Unknown kind ${refl.kind} for "${refl.name}", skipping`,
      );
      return undefined;
  }
}
...
function convertClass(refl: TdReflection, modulePath: string): TsClass {
  const ext = refl.extendedTypes?.length
    ? typeToString(refl.extendedTypes[0])
    : undefined;
...
function convertEnum(refl: TdReflection, modulePath: string): TsEnum {
  const members = (refl.children ?? [])
    ...
    .map((c) => ({
      name: c.name,
      value:
        c.type?.type === "literal" && c.type.value != null
          ? String(c.type.value)
          : undefined,
    }));
