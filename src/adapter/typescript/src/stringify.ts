  ...
  switch (type.type) {
    ...
    case "predicate":
      return type.targetType
        ? `${type.name} is ${typeToString(type.targetType)}`
        : type.name;
...
export function buildSignature(refl: TdReflection): string {
  ...
  const tp = refl.typeParameters?.length
    ? typeParamsToString(refl.typeParameters)
    : "";
  ...
  if (kind === ReflectionKind.Class) {
    const ext = refl.extendedTypes?.length
      ? ` extends ${typeToString(refl.extendedTypes[0])}`
      : "";
  ...
  if (kind === ReflectionKind.Function) {
    ...
    const sigTp = sig.typeParameters?.length
      ? typeParamsToString(sig.typeParameters)
      : "";
    const params = (sig.parameters ?? []).map((p) => {
  ...
  if (kind === ReflectionKind.Enum) {
    const members = (refl.children ?? [])
      ...
      .map((c) => {
        if (c.type && c.type.type === "literal" && c.type.value != null) {
